...
// Namen und Reihenfolge der drei Ebenen stehen in DESIGN 8 und in
// schule.stufen. Wer schule.stufen übergibt, bekommt die Namen von dort.
...
/**
 * Baum mit Wurzel, Stamm und Krone (DESIGN 8).
 * Nur auf der Stufen-Seite, nicht als Schmuck.
 *
 * @param {object} optionen
 *   aktuelleStufe  id der Stufe, die als "du bist hier" gilt (optional,
 *                  die App fragt das Kind nicht, DESIGN 8)
 *   hervorheben    id der Zielstufe, die aufleuchtet
 *   stufen         Liste aus schule.stufen (optional)
 *   onEbene        Funktion(id), öffnet Rechte und Freiheiten der Ebene
 */
export function stufenBaum(optionen = {}) {
  const { aktuelleStufe = null, hervorheben = null, stufen = STUFEN_STANDARD, onEbene = null } = optionen;
  const liste = Array.isArray(stufen) && stufen.length === 3 ? stufen : STUFEN_STANDARD;

  const svg = document.createElementNS(NS_SVG, 'svg');
  svg.setAttribute('viewBox', '0 0 200 220');
  svg.setAttribute('class', 'stufen-baum');
  svg.setAttribute('role', 'group');
  svg.setAttribute('aria-label', 'Baum der drei Stufen');

  // Von oben nach unten: Krone, Stamm, Wurzel.
  const ebenen = [
    { stufe: liste[2], form: 'M100 10a58 58 0 0 1 48 90H52A58 58 0 0 1 100 10z', schriftY: 62 },
    { stufe: liste[1], form: 'M76 106h48v62H76z', schriftY: 142 },
    { stufe: liste[0], form: 'M100 174c26 0 46 14 46 30H54c0-16 20-30 46-30z', schriftY: 196 }
  ];

  const boden = document.createElementNS(NS_SVG, 'line');
  ...
  for (const ebene of ebenen) {
    const gruppe = document.createElementNS(NS_SVG, 'g');
    gruppe.setAttribute('class', 'stufen-ebene');
    gruppe.setAttribute('data-ebene', String(ebene.stufe.id));
    gruppe.setAttribute('role', 'button');
    gruppe.setAttribute('tabindex', '0');
    gruppe.setAttribute('aria-label', 'Stufe ' + ebene.stufe.id + ', ' + ebene.stufe.name);
    if (String(aktuelleStufe) === String(ebene.stufe.id)) gruppe.setAttribute('aria-current', 'true');
    if (String(hervorheben) === String(ebene.stufe.id)) gruppe.setAttribute('data-hervorheben', 'true');

    const form = document.createElementNS(NS_SVG, 'path');
    form.setAttribute('d', ebene.form);
    form.setAttribute('class', 'stufen-form');

    const schrift = document.createElementNS(NS_SVG, 'text');
    ...
    schrift.textContent = ebene.stufe.name;

    gruppe.append(form, schrift);

    if (onEbene) {
      gruppe.addEventListener('click', () => onEbene(ebene.stufe.id));
      gruppe.addEventListener('keydown', (ereignis) => {
        if (ereignis.key === 'Enter' || ereignis.key === ' ') {
          ereignis.preventDefault();
          onEbene(ebene.stufe.id);
        }
      });
    }
